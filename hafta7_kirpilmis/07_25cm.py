...
conn = sql.connect('seramik25.db')
cursor = conn.cursor()


img = cv2.imread("ornek_2_25cm - Kopya.jpg")
height = img.shape[0]
width = img.shape[1]
...
circle = cv2.HoughCircles(gray,cv2.HOUGH_GRADIENT,dp=1,minDist=700,param1=200,param2=30,minRadius=20,maxRadius=800)
...
cursor.execute(''' CREATE TABLE IF NOT EXISTS LabData2 (id INTEGER PRIMARY KEY, L REAL, a REAL, b REAL) ''')
cursor.execute(''' INSERT INTO LabData2 (L,a,b) VALUES (?,?,?)  ''', (L,a,b))

#tabloyu bi dataframe'e oku:
df = pd.read_sql_query("SELECT * FROM LabData2;", conn)

df.to_excel('25LabData.xlsx', index = False)
